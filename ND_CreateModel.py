...
from keras.layers import Input, Dense, Reshape, Flatten, Dropout, multiply, GaussianNoise, MaxPooling2D
...
ModelName = 'ALOCC_Model.model'
ModelWeightsName = 'ALOCC_Model_{}.h5'

Images_base_path = r"C:\Users\me919585\PycharmProjects\imageload\ScreenshotsSortedv2\ProductBlock"
Images_folder = r"\ND"
...
class ALOCC_Model():
    def __init__(self,
        ...
        if self.dataset_name == 'mnist':
            ...
            pic_size_X = self.input_height  # 300
            pic_size_Y = self.input_width  # 600
            ...
            train_generator = datagen_train.flow_from_directory(base_path + Images_folder,
            ...
            X_train = X_train / 255
            self.data = X_train.reshape(-1, pic_size_X, pic_size_Y, colors)
    ...
    def build_generator(self, input_shape):
        ...
        x = UpSampling2D((2, 2))(x)
        x = Conv2D(colors, kernel_size=5, activation='sigmoid', padding='same')(x)
        return Model(image, x, name='R')

    def build_generatorAlternate(self, input_shape):
        """Build the generator/R network.
    ...
    def build_model(self):
        image_dims = [self.input_height, self.input_width, colors]
        ...
        #self.generator = self.build_generatorAlternate(image_dims)
    ...
    def save(self, step):
        ...
        modelWeightsStep_name = ModelWeightsName.format(step)
        self.adversarial_model.save_weights(os.path.join(self.checkpoint_dir, modelWeightsStep_name))
        self.adversarial_model.save(os.path.join(self.checkpoint_dir, ModelName))


if __name__ == '__main__':
    model = ALOCC_Model(dataset_name='images', input_height=Image_Height, input_width=Image_Width, output_height=Image_Height, output_width=Image_Width)
    model.adversarial_model.save(os.path.join(model.checkpoint_dir, ModelName))
    model.train(epochs=epochs, batch_size=20, sample_interval=100)
